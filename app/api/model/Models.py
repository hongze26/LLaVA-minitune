...
class Weibo(db.Model,BaseModel):
    ...
    # 转发ID
    retweet_id = Column(String(32), nullable=False,comment='转发ID')
    #weibo链接
    weibo_url = Column(String(128), nullable=False,comment='weibo链接')
    def __init__(self, bid, user_id, screen_name, text,topics,at_users,pics
    ,video_url,location,created_at,source,attitudes_count,comments_count,reposts_count,retweet_id,weibo_url):
...
class Comments(db.Model,BaseModel):
    ...
    rootid = Column(String(64),comment='根评论ID')
     # 微博链接
    weibo_url = Column(String(64),comment='微博链接')


    def __init__(self, mid,screen_name,like_count, floor_number, follow_count,source,comment,gender,rootid,weibo_url):
