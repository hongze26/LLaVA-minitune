...
def start():
    ...
    # 添加采集定时任务
    print('添加采集定时任务')
    scheduler.add_job(spider, 'interval',seconds=3600* 1)
    # 添加生成词云定时任务
    print('生成词云定时任务')
    scheduler.add_job(build_wordcloud, 'interval',seconds=3600* 1)
    # # 添加构建情感分析定时任务
    print('构建情感分析定时任务')
    scheduler.add_job(build_nlp, 'interval',seconds=3600* 1)
    scheduler.start()
